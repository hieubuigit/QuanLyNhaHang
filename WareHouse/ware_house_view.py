...
class WareHouseView:
    ...
    def ui_right_content_view(self):
        ...
        self.tv["columns"] = ("id", "name", "unit", "price", "quantity", "capacity",
                              "alcohol", "type", "image", "create_date", "update_date")
        ...
        self.tv.column("image", anchor="center", width=100)
        ...
        self.tv.heading("image", text="Hình ảnh")
        ...
        title_option = CTkLabel(option_fr, text="Thêm mới", font=heading2)
    ...
    def insert_row_treeview(self):
        ...
        if products:
            for p in products:
                ...
                if p.image:
                    image_decode = p.image + b'=' * (-len(p.image) % 4)
                    de = base64.b64decode(image_decode)
                    image = Image.open(io.BytesIO(de))
                    image.load()
                    photo = ImageTk.PhotoImage(image)
                    img_table = ImageTk.PhotoImage(Image.open("../assets/ic_table_visible.png").resize((50, 50)))

                    self.tv.insert("", "end", iid=p.id, text=p.id, image=img_table,
                                   values=(p.id, p.name, p.unit, p.price, p.quantity,
                                           p.capacity, p.alcohol, p.productType, "test", p.createdDate, p.updatedDate))
                    self.tv.image_references = [img_table]
